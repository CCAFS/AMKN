...
if (isset($_POST['pn'])) {
  ...
  $sql1 = "SELECT a.*, b.name as name_sub FROM dt_deliverables a INNER JOIN dt_subcategories b ON (a.subcategory_id = b.id) INNER JOIN dt_categories c ON (b.category_id = c.id) WHERE c.id= " . $category . " $where ORDER BY name_sub, name $limit";
  $results = $wpdb->get_results($sql1);
}
?>
<?php if (count($results)):?>
<div style="font-size: 30px;">Results for Platforms and Tools</div>
<?php foreach ($results as $res): ?>
  <div class="clearfix" style="margin-bottom: 15px;width: 100%; cursor: pointer;" onclick="deliverableChosen(<?php echo $res->id ?>)">
    <div class="label-subcategory <?php echo ($res->external)?'external':''?>"><span> <?php echo ($res->external)?'External<br>'.$res->name_sub:$res->name_sub ?></span></div>
    <div class="content-category <?php echo ($res->external)?'external':''?>" > 
      <?php echo $res->name ?>
