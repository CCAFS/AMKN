...
//            $coveragepopulatedplace = GetNameAdministrativeDivision($record['coveragepopulatedplace']);
            ...
            $this->addChild($this->working_node, 'dc:creator', $record['creator']);                             //Creators of the item (person, organization, or service) - (Lead of Project (last, middle, name))
            $this->addChild($this->working_node, 'cg.creator.id', '');                                          //Used if ORCID, SCOPUS, or other type of creator ID scheme is in use. Used in parallel with cg.creator.ID.type - ()
            $this->addChild($this->working_node, 'cg.creator.id.type', '');                                     //Used to indicate the type of Creator ID – ex: SCOPUS, ORCID, etc. - ()
//            $this->addChild($this->working_node, 'dc:subject', $record['subject']);                             //Subject matter of the research, technologies tested, etc. - (Keywords)
            $this->addChild($this->working_node, 'cg.subject.agrovoc', '');                                     //AGROVOC subject matter or research area - ()
            $this->addChild($this->working_node, 'cg.subject.domain-specific', '');                             //Subject matter or research area from domain-specific vocabularies, if missing from AGROVOC - ()
//            $this->addChild($this->working_node, 'dc:description.abstract', $record['description']);            //Abstract or longer description of the item - (Abstract)
            $this->addChild($this->working_node, 'dc:publisher', '');                                           //Entity responsible for publication, distribution, or imprint - ()
//            $this->addChild($this->working_node, 'dc:contributor', $record['contributor']);                     //Person, organization, or service making contributions to resource content; Research Centers and offices with which creator(s) are affiliated - (Lead Institution)
            $this->addChild($this->working_node, 'cg.contributor.crp', '');                                     //CGIAR Research Program with which the research is affiliated - ()
...
//            $this->addChild($this->working_node, 'cg.contributor.partnerid', $record['contributorpartnerid']);  //Partners, funding agencies, other CGIAR centers - (Implementing Institutions)
//            $this->addChild($this->working_node, 'cg.contributor.project', $record['contributorproject']);      //Name of project with which the research is affiliated - (Project Name)
            $this->addChild($this->working_node, 'dc:date', $record['date']);                                   //Publication, creation or issue date - the formal publication date in case of manuscripts - ()
//            $this->addChild($this->working_node, 'cg.date.embargoenddate', $record['embargoenddate']);          //Date Project Ends and/or Final Research Output is Public - ()
            $this->addChild($this->working_node, 'dc:type', 'data set');                                        //Nature or genre of item/content; e.g., poster, data set - ()
            $this->addChild($this->working_node, 'dc:format', 'Web');                                         //File format of item e.g.: PDF; jpg etc. - () 
            $this->addChild($this->working_node, 'dc:identifier', 'oai:amkn.org:'.$record['id']);                         //Unambiguous reference to resource such as doi, uri - ()
            $this->addChild($this->working_node, 'dc:identifier.citation', '');                                 //Human-readable, standard bibliographic citation for the item - ()
            $this->addChild($this->working_node, 'dc:source', '');                                              //Journal/conference title; vol., no. (year) - ()
            $this->addChild($this->working_node, 'dc:language', 'English');                                     //Language of the item; use ISO 639-1 (alpha-2) or ISO 639-2 (alpha-3). - ()
//            $this->addChild($this->working_node, 'dc:relation', $record['relation']);                           //Supplemental files, e.g. data sets related to publications or larger “whole” - (Suppplemental Information File)
            $this->addChild($this->working_node, 'dc:coverage', $record['coverage']);                           //Geospatial coordinates, countries, regions, sub-regions, chronological period - (Latitude and Longitude)
            $this->addChild($this->working_node, 'dc:coverage.region', '');                                     //FAO or WorldBank list - ()
//            $this->addChild($this->working_node, 'dc:coverage.country', $coveragecountry);                      //Country/countries related to the data which was collected in resource.  ISO 3166 Standard - (Country)
//            $this->addChild($this->working_node, 'dc:coverage.subnational', $coveragesubnational);              //State, Province, County, District, etc.  GAUL (FAO) - (District/State/Province Level)
//            $this->addChild($this->working_node, 'dc:coverage.populatedplace', $coveragepopulatedplace);        //City, Town, Village, etc. - (Village Level)
//            $this->addChild($this->working_node, 'dc:coverage.start-date', $record['coveragestartdate']);       //Chronological period: start date of activity described in resource - (Trial implementing period start date)
//            $this->addChild($this->working_node, 'dc:coverage.end-date', $record['coverageenddate']);           //Chronological period: end date of activity described in resource - (Trial implementing period end date)
            $this->addChild($this->working_node, 'dc:rights', 'Open Access');                               //Rights, licensing, permission statement - (License)
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    }
