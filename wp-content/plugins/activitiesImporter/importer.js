...
for (var i = 0, len = activities.length; i < len; i++) {
  ...
  cont = activities[i].getElementsByTagName("deliverables")[0].getElementsByTagName("deliverable");
  for (var j = 0, lenj = cont.length; j < lenj; j++) {
    output += '<ccafs:deliverableTitle>' + cont[j].getElementsByTagName("title")[0].childNodes[0].data + '</ccafs:deliverableTitle>\n';
    output += '<ccafs:deliverableType>' + cont[j].getElementsByTagName("type")[0].childNodes[0].data + '</ccafs:deliverableType>\n';
  }

  cont = activities[i].getElementsByTagName("objectives")[0].getElementsByTagName("objective");
  for (var j = 0, lenj = cont.length; j < lenj; j++) {
...
function saveFile(output) {
  $("#loading").show();
  $.ajax({
    url: templateUrl + "/activitiesImporter/createXML.php",
    type: "POST",
    data: {dataxml: output},
