...
function findPointsInPolygon(extent, evt, filtParam) {
  filtParam = filtParam || '';
//  extentLocal = '';
  if (extent != '')
    extentLocal = extent;
  ...
  filtType = filtParam;
  dojo.forEach(dataLayer.graphics, function(graphic) {
    if (extentLocal.contains(graphic.geometry)) {
      var li = getListingContent(graphic.attributes.id);
      if (li != '')
        results.push(li);
    }
  });
